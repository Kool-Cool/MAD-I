...
def registration():
    ...
    if request.method == "POST":
        ...
        new_user = User(
            username=username, password=password, email=email, role="influencer"
        )
...
def dashboard():
    if "user_name" in session and "role" in session:
        if session["role"] == "influencer":
            ...
            # print(user_id)
            influencer_id = (
                Influencer.query.filter_by(user_id=user_id).first().influencer_id
            )
    ...
    if "user_name" in session and "role" in session:
        if session["role"] == "influencer":
            ...
            if request.method == "POST":
                ...
                old_ad_requt = AdRequest.query.filter_by(
                    ad_request_id=show_info["ad_request_id"]
                ).first_or_404()
                ...
                if show_info["negotiation_id"] is not None:
                    old_nego = Negotiation.query.filter_by(
                        negotiation_id=show_info["negotiation_id"]
                    ).first_or_404()
