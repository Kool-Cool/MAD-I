...
def admin_registration():
    ...
    if request.method == "POST":
        ...
        add_admin = User(
            username=new_username, password=new_password, email=new_email, role=new_role
        )
    ...
    if request.method == "POST":
        ...
        if admin_data:
            if (
                ...
                and admin_pass == admin_data["password"]
                and admin_data["role"] == "admin"
            ):
                session["user_name"] = (
                    admin_name  # We are using UserName (so that no two User (admin/contractor/influncer) login at same)
                )
                session["role"] = "admin"
...
def admin_dashboard_data():
    ...
    total_ad_requests_negotiation = AdRequest.query.filter_by(
        status="negotiation"
    ).count()
...
def flag_user(user_id):
    if "user_name" in session and "role" in session:
        if session["role"] == "admin":
            ...
            flag = UserFlag(
                flagged_by=session["user_id"], user_id=user_id, reason=reason
            )
            db.session.add(flag)
            db.session.commit()
...
def flag_campaign(campaign_id):
    if "user_name" in session and "role" in session:
        if session["role"] == "admin":
            ...
            reason = data.get("reason")
            flag = CampaignFlag(
                flagged_by=session["user_id"], campaign_id=campaign_id, reason=reason
            )
            db.session.add(flag)
            db.session.commit()
