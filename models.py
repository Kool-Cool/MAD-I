...
class User(db.Model):
    ...
    flags = db.relationship('Flag', foreign_keys='Flag.flagged_by', backref='flagger', lazy=True)
    flagged = db.relationship('Flag', foreign_keys='Flag.user_id', backref='flagged_user', lazy=True)
...
class Campaign(db.Model):
    ...
    flags = db.relationship('Flag', backref='campaign', lazy=True)
...
def init_db(app: Flask):
    with app.app_context():
        ...
        if not User.query.first():
            ...
            sponsor_data = Sponsor(user_id=sponsor.user_id, company_name='Sponsor Company', industry='Technology', budget=10000.00)
            influencer_data = Influencer(user_id=influencer.user_id, name='Influencer Name', category='Lifestyle', niche='Travel', reach=5000)
